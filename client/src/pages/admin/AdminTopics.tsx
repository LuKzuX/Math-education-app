...
function AdminTopics() {
  ...
  const handleSubmit = async () => {
    if (!title || !description || !topicIcon) {
    ...
    <div className="min-h-screen bg-[#0a0f1e] px-4 sm:px-6 md:px-8 py-10">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="relative pl-5 mb-8">
          ...
          <p className="text-sm text-slate-500 mt-1">Create a new topic for this path</p>
        </div>
        ...
        <div className="bg-[#0f1629] border border-slate-800 rounded-xl p-5 sm:p-6 space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              placeholder="Short tagline shown on the topic list"
        ...
        <div className="mt-8">
          ...
          {topics.length === 0 ? (
            <p className="text-sm text-slate-500">No topics yet for this path.</p>
          ) : (
            <div className="space-y-2">
              {topics.map((topic) => (
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  onClick={() => navigate(`/admin/topics/${topic.topic_id}/challenges`)}
                  onKeyDown={(e) => e.key === "Enter" && navigate(`/admin/topics/${topic.topic_id}/challenges`)}
                  className="flex items-center justify-between gap-4 bg-[#0f1629] border border-slate-800 rounded-lg px-4 py-3 cursor-pointer hover:border-cyan-400/60 transition-colors"
                >
                  ...
                  <span className="shrink-0 font-['JetBrains_Mono'] text-[11px] tracking-widest uppercase text-cyan-300">
                    Manage challenges »
                  </span>
                </div>
              ))}
